...
#pragma mark----- 上传服务器了
- (void)upLoadServiceRecordBtnClieckedAction:(UIButton *)sender{
    
}
//点击 重录， 输入框中的 录音按钮
- (void)goToRecorderClickedActionBySender:(UIButton *)sender{
 

//    _resetButton.hidden = YES;
//    _playButton.hidden = YES;
//    _recordImgView.hidden = NO;
//    _recordImgView.image = [UIImage imageNamed:@"microphone"];
...
//        make.size.mas_equalTo(CGSizeMake( 240 * kSAScreenWidthRatio(), 40  * kSAScreenHeightRatio()));
//        make.left.equalTo(self.reasonMaskingView.mas_left).offset(35 * kSAScreenWidthRatio());
//        make.bottom.equalTo(self.reasonMaskingView.mas_bottom).offset(-40  * kSAScreenHeightRatio());
//    }];
}
...
- (UIImageView *)recordImgView{
    if (!_recordImgView) {
        _recordImgView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"u4558.png"]];
        _recordImgView.frame = CGRectMake(60, 200, 60, 60);
...
- (UIButton *)resetButton{
    if (!_resetButton) {
        ...
        _resetButton.frame = CGRectMake(40, 300, (self.frame.size.width - 40 * 3) / 2.0, 40);
...
- (UIButton *)playButton{
    if (!_playButton) {
        ...
        _playButton.frame = CGRectMake(40, 160, 120, 40);
...
-(UIButton *)voiceButton{
    if (!_voiceButton) {
        ...
        _voiceButton.frame = CGRectMake(40, 100, 120, 40);
        [_voiceButton setTitle:NSLocalizedString(@"按住录入语音", nil) forState:UIControlStateNormal];
