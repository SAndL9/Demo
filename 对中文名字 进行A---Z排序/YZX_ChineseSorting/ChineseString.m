...
#warning mark 将ChinesString的属性，全部赋给Person的属性
         NSString *string = ((ChineseString*)object).string;
        Person *p = [[Person alloc] init];
        p.name = string;
        p.age = ((ChineseString*)object).age;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        Person *p = [stringArr objectAtIndex:i];
        chineseString.string=[NSString stringWithString:p.name];
#warning mark 将Person的属性，全部赋给ChinesString
        chineseString.age = p.age;
...
+(NSMutableArray*)SortArray:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++){
        Person *p = [tempArray objectAtIndex:i];
        [result addObject:((ChineseString*) p.name).string];
